...
def loss_func(loss_mask, dec_moe_loss, enc_moe_loss, output_tensor):
    lm_loss_ = output_tensor.float()
    lm_loss = torch.sum(
        lm_loss_.view(-1) * loss_mask.reshape(-1)) / loss_mask.sum()

    loss = lm_loss + dec_moe_loss + enc_moe_loss
    averaged_losses = average_losses_across_data_parallel_group([lm_loss])

    return loss, {'lm loss': averaged_losses[0],'dec moe loss': dec_moe_loss, 'enc moe loss': enc_moe_loss}
...
def forward_step(data_iterator, model):
    ...
    # Modifying the code below for MoE, copied logic from pretrain_gpt.py
    ...
    output_tensor, dec_moe_losses, enc_moe_losses  = model(tokens_enc,
                          tokens_dec,
                          enc_mask,
                          dec_mask,
                          enc_dec_mask,
                          tokentype_ids=None,
                          lm_labels=lm_labels)
    # Did not introduce MoS loss yet
    dec_moe_loss = sum_loss(dec_moe_losses, args.moe_loss_coeff)
    enc_moe_loss = sum_loss(enc_moe_losses, args.moe_loss_coeff)
    # print('LOLA: t5 output_tensor from model:', output_tensor)

    return output_tensor, partial(loss_func, loss_mask, dec_moe_loss, enc_moe_loss)

def sum_loss(losses, coeff=1):
    losses = []
    for loss_val in losses:
        if loss_val is not None:
            losses.append(loss_val)
    tot_loss = sum(losses) * coeff
    return tot_loss
