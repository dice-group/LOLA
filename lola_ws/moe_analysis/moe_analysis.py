...
with open(args.output_dir / f'row_data.{args.lang}.data', 'w') as datafile, open(args.output_dir / f'token_experts.{args.lang}.dat', 'w', newline='') as csvfile:
    ...
    for i, row in enumerate(tqdm.tqdm(itertools.islice(dataset, args.rows_limit), desc=args.lang, total=args.rows_limit)):
        experts_info = []
        ...
        for token in range(tokens):
            writer.writerow([args.lang, i, token] + [int(experts_info[layer][token]) for layer in range(layers)])
