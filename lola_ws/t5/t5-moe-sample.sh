...
### Using configuration values from here: https://github.com/dice-group/LOLA-Megatron-DeepSpeed/blob/main/examples/pretrain_t5.sh
### We keep some config same as the GPT3 if they do not really have to change, and also due to the fact that they were suggested for an MoE model
...
# t5 Specific args
## Sets the inner dimension of the "key" and "value" matrices of all attention mechanisms in the model.
## For BERT and GPT this defaults to the hidden size divided by the number of attention heads, but can be configured for T5.
KV_CHANNELS=64
## Sets the hidden size in the feed-forward networks within a transformer layer. For BERT and GPT this defaults to
## 4 times the transformer hidden size, but can be configured for T5.
